...
function DynamoDBExportCSV(awsAccessKeyId, awsSecretAccessKey, awsRegion) {
  ...
  function writeTableToCsv(table, filter, columns, totalSegments, segment, compressed, filesize, s3Bucket, s3Path, callback) {
    var query = {
      TableName: table,
      Segment: segment,
    ...
    async.series([
      ...
      function(done) {
        ...
        for (var i = 0; i < totalSegments; i++) {
          parallelScanFunctions.push(function(worker) {
            return function(done) {
              ...
              child.on('exit', function(code) {
                self.emit(infoEvent, logPrefix + 'Exit Code ' + code); done();
              });
            };
          }(i)
  ...
  // Public export table function
  this.exportTableWorker = function exportTableWorker(table, filter, columns, totalSegments, segment, compressed, filesize, s3Bucket, s3Path, callback) {
    if (!filesize) {
      filesize = 250;
    }
    ...
    async.series([
      ...
      // Scan the table
      function(done) {
        writeTableToCsv(table, filter, columns, totalSegments, segment, compressed, filesize, s3Bucket, s3Path, done);
      }
    ], callback);
  };
